...
void Button::setup()
{
	...
	m_buttonText.setCharacterSize(80.0f);
	m_buttonText.setFillColor(Color::White);
	
	m_button.setSize(Vector2f(m_textContents.size() * (m_buttonText.getCharacterSize() / 4.0f) + m_padding.x, m_buttonText.getCharacterSize() +  m_padding.y));
	...
	m_buttonText.setPosition(m_position + Vector2f(0.0f,-15.0f));
...
bool Button::hover(Event& t_event, RenderWindow& t_window)
{
	if (t_event.type == Event::MouseMoved)
	{
		...
		{
			m_button.setFillColor(Color::Blue);
			return true;
		}
		else
		{
